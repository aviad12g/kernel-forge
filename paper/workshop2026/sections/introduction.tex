...
The contribution is an evaluation design rather than a generation model:
\begin{itemize}
  \item a contract-valid persistent lifecycle for official \code{ModelNew}
  tasks, with frozen initialization provenance and source hashes;
  \item a screen-then-confirm protocol using randomized paired blocks across
  fresh processes and a practical-margin promotion test;
  \item fail-closed null, known-slowdown, and lifecycle controls that must pass
  before candidate screening; and
  \item an artifact ledger that preserves the evidence needed to audit both a
  candidate and the evaluator that promoted it.
\end{itemize}
